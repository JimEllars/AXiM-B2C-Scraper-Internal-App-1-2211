  ...
  const handleAddProxy = async (e) => {
    ...
    try {
      ...
      if (window.addNotification) {
          window.addNotification('Proxy Added', `Successfully linked provider: ${newProxy.provider}`, 'success');
      }
      ...
      if (window.addNotification) {
          window.addNotification('Proxy Error', `Failed to link provider: ${newProxy.provider}`, 'error');
      }
    } finally {
      setAdding(false);
    }
